...
def makeBOMfile(parts,outDir):
    def getInventoryInfo(outDir,part):
        # Prepare the inventory file.
        inventoryFile = outDir + "//inventory.csv"
    ...
    fileName = outDir + "//BoMforOctopart.csv"
    with open(fileName,'w') as csvfile:
        ...
        newInventoryFilename = outDir + '//inventory_' +time.asctime().translate(trans)+'.csv'
        ...
        with open(newInventoryFilename, 'w',newline='') as csv_file:
            ...
            for part_number, components in parts.items():
                if part_number != 'None':
                    ...
                    quantityHave,description = getInventoryInfo(outDir,part_number)
                    if (quantityHave < 0):
                        logger.info('Part number: {} is not listed in the inventory file.'.format(part_number))
                        # put the number of components figured out from the Kicad xml bom tool.
                        write_row(csvwriter,part_number,quantityNeed,components,'')
                    elif (quantityHave-quantityNeed >= 0):
                            logger.info('Part number: {} have {} on hand, need {}. '.format(part_number,quantityHave,quantityNeed))
                            ...
                            s = '****' if quantityHave - quantityNeed == 0 else str(quantityHave-quantityNeed)
                            newInventoryCSV.writerow((str(part_number),s,description))
                    else:
                        quantityOrder = abs(quantityHave-quantityNeed)
                        logger.info('Need to order {} more of part number: {}'.format(quantityOrder,part_number))
                        write_row(csvwriter,part_number,quantityOrder,components,description)
                        # Not sure how many ordered so make it obvious where need to edit...
                        newInventoryCSV.writerow((str(part_number),'****',description))
                else:
                    logStr = '{} Skipping: {} part number: {} '.format(refStr,part_number)
####################################################################################
